...
searchP <- function(x , pattern, caption=TRUE, before=FALSE, after=FALSE, ignore.case=TRUE, ...){

   ## x should be list (sentences or tables) OR supplementary table -  is.list(data.frame()) = TRUE
   if(class(x)!="list" ){
         y <- attr(x, "label")
