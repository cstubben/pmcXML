...
## Some text before table will be dropped, but only a few  PMC XML files with table-wrap inside <p> tag

searchHTML <- function(doc, pattern, before=0, after=0, path="//p[not(descendant::table-wrap)]", ignore.case=TRUE, htmlSection="h2", ...){
   ...
   for(i in 1:length(x)){
      ...
      y <- searchXML(doc2, pattern, before, after, path, ignore.case, ...)     
      free(doc2)
